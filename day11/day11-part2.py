...
while True:
    ...
    flashed_in_this_step = run_step(octopuses)
    total_flashes += flashed_in_this_step

    # print(flashed_in_this_step, len(octopuses))

    if flashed_in_this_step == len(octopuses):
        print(f'All octopuses flashed in step {iteration}')
        break
    # for row_no in range(len(rows)):
    #     print([octopuses[(row_no, col_no)] for col_no in range(len(rows[0]))])

print(f'After step {iteration}, there has been {total_flashes} flashes')